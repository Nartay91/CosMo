...
const ModalBranch = ({ onClose, onSave }) => {
  const [title, setTitle] = useState(""); // переименовано из name в title
  const [address, setAddress] = useState("");
  const [selectedManager, setSelectedManager] = useState("");
  const [showManagerModal, setShowManagerModal] = useState(false);
  const [showTooltip, setShowTooltip] = useState(false);
  const [isManualEntry, setIsManualEntry] = useState(false); // Флаг для ручного ввода
  const inputRef = useRef(null);
  const timeoutId = useRef(null);

  // Используем manager_full_name из филиалов, если он есть
  const existingManagers = useBranchStore.getState().branches.flatMap(
    (branch) => (branch.manager_full_name ? [branch.manager_full_name] : [])
  );
  ...
  const handleSave = () => {
    // Передаём объект с полями, соответствующими API: { title, address, manager }
    onSave({ title, address, manager: selectedManager });
    onClose();
  };
    ...
    <div className="modal1">
      <div className="modal-content">
          ...
          <div className="name_info">
            ...
            <div className="manager-input">
              <input
                ...
                readOnly={!isManualEntry} // Разрешаем ввод только при ручном режиме
              ...
              >
                <img src={plus} alt="Добавить управляющего" />
                {showTooltip && (
        ...
        <div className="buttons_division">
          ...
          <button className="submits" onClick={handleSave}>
            Сохранить
          </button>
        </div>
      </div>
